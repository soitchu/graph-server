...
export class XYExtension extends GraphExtension {
    ...
    counter = new Uint32Array(new SharedArrayBuffer(Uint32Array.BYTES_PER_ELEMENT * 2));
    constructor(window, config, workerCode, usesOnlyX, workerPath){
        super(window);
        ...
        this.workerCode = workerCode === null || workerCode === void 0 ? void 0 : workerCode.toString();
    ...
    async iniWorkers() {
        const self = this;
        const hasDedicatedWorker = this.WORKER_PATH !== XYExtension.XY_WORKER_PATH && this.WORKER_PATH !== XYExtension.XLine_WORKER_PATH;
        let workerCode = await (await fetch(this.WORKER_PATH)).text();
        if (!hasDedicatedWorker) {
            let hasWorkerCode = !!this.workerCode;
            let code = hasWorkerCode ? this.workerCode : this.getWorkerCode.toString();
        ...
        for(let i = 0; i < this.workers.length; i++){
            if (!(this.workers[i] instanceof Worker)) {
                ...
                this.workers[i].onerror = function(ev) {
            ...
            this.workers[i].onmessage = function(event) {
                ...
                if (responseId == self.responseId) {
                    ...
                    if (self.currentResponses == self.workers.length) {
                        ...
                        if (self.wasInterrupted === false && self.graphHasMoved() === false) {
                            self.workerCallback();
                        } else {
                            console.warn("Skipping rendering because the graph was interrupted or moved");
                        }
    ...
    graphHasMoved() {
        return !(this.startScale === this.window.graphInstance.scale && this.startTranslate[0] === this.window.graphInstance.translate.x && this.startTranslate[1] === this.window.graphInstance.translate.y);
    }
    resizeSharedBuffer() {
    ...
    async redraw() {
        ...
        return new Promise((resolve, reject)=>{
            ...
            if (this.shouldAnimateRedraws && !this.usesOnlyX) {
                ...
                sharedArray.set(this.ctx.getImageData(0, 0, this.canvas.width, this.canvas.height).data);
            }
