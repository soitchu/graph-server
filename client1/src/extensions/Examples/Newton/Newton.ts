...
export class Newton extends XYExtension {
    ...
    getWorkerCode() {
        ...
        function float2(x: number, y: number) {
            return [x, y];
        }
        ...
        function cdiv(z1: number[], z2: number[]) {
        ...
        function cmul(z1: number[], z2: number[]) {
        ...
        function cadd(z1: number[], z2: number[]) {
            const a = z1[0], b = z1[1], c = z2[0], d = z2[1];
        ...
        function cdif(z1: number[], z2: number[]) {
            const a = z1[0], b = z1[1], c = z2[0], d = z2[1];
        ...
        function Derivative(z: number[]) {
            return cmul([3, 0], cmul(z, z));
        }

        function cal(x: number, y: number): Array[number] {
            // modify result
