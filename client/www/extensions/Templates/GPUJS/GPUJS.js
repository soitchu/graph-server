...
export class GPUJS extends GraphExtension {
    ...
    webglCanvas;
    webglCtx;
    constructor(gw, func, outputSize){
        super(gw);
        this.kernelFunction = func;
        this.outputSize = outputSize;
        this.window = gw;
        this.webglCanvas = new OffscreenCanvas(this.window.graphInstance.width, this.window.graphInstance.height);
    ...
    async redraw() {
        ...
        const out = this.kernel(graph.scale, graph.scaleY, graph.translate.x, graph.translate.y, graph.width);
    ...
    async resizeCallback(width, height) {
        ...
        this.kernel = this.gpu.createKernel(this.kernelFunction).setGraphical(true).setOutput([
            width,
            height
        ]);
    }
