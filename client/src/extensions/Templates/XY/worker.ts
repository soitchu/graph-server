...
function changeImageBufferXAtomics() {
    ...
    const xScale = payload.config.xScale ?? 1;
    ...
    while (count < maxCount) {
        ...
        if (shouldAnimateRedraws) {
            for (let i = 0; i < height; i++) {
                ...
                canvasImageData.set([0, 0, 0, 0], offset);
            }
        }
    ...
    postMessage([payload.responseId, wasInterrupted]);
}
...
function changeImageBuffer() {
    ...
    for (let i = payload.x.start; i < payload.x.end; i++) {
        ...
        for (var j = 0; j < canvasHeight; j++) {
            ...
            const color = cal(
                i / scale - translateX,
                -(j / scale - translateY) / payload.scaleY
            );
    ...
    postMessage([payload.responseId, false]);
}
...
onmessage = async function (event: MessageEvent) {
    ...
    if ("alpha_strength" in payload.config) {
        pixelOpacity = payload.config.alpha_strength as number;
    }
