...
export class Attractor extends GraphExtension {
  ...
  async redraw() {
    ...
    this.modelCoords = new Float64Array(
      config.endModelPoints - config.startModel + 1
    );
    ...
    const n = (nt: number, pt: number) => {
      return (
        (1 - config.theta1) * nt +
        config.theta1 *
          Math.pow(quarterRoot(pt / Math.pow(c1, 2)) - Math.sqrt(pt), 2)
      );
    };
    ...
    const p = (nt: number, pt: number) => {
      return (
        (1 - config.theta2) * pt +
        config.theta2 *
          Math.pow(quarterRoot(nt / Math.pow(c2, 2)) - Math.sqrt(nt), 2)
      );
    };
    ...
    let nLast = this.config.nIni ?? nIni;
    let pLast = this.config.pIni ?? pIni;
