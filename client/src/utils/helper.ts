...
export function drawGraphXOld(canvas: HTMLCanvasElement | OffscreenCanvas, ctx: CanvasRenderingContext2D | OffscreenCanvasRenderingContext2D, lineData: Float64Array) {
...
export function drawGraphX(
    canvas: HTMLCanvasElement | OffscreenCanvas,
    ctx: CanvasRenderingContext2D | OffscreenCanvasRenderingContext2D,
    lineData: Float64Array,
    legacyMode = true
) {
    const height = canvas.height;
    const viewHeight = 2 * height;
    ...
    for (let i = 0; i < graphNums; i++) {
        ...
        for (let j = start; j < end; j += sectionLength) {
            ...
            let x1 = lineData[j + offset],
                y1 = lineData[j + offset + 1],
                x2 = lineData[j + offset + 2],
                y2 = lineData[j + offset + 3];
...
function pointOnLine(m: number, c: number, x: number) {
    return m * x + c;
}
...
export function drawLines(
    ...
    for (let i = 0; i < coords.length; i += 4) {
        const index = i;
        const startCoords = graphInstance.graphToCanvasCoords(
            coords[index],
            coords[index + 1]
        );
        const endCoords = graphInstance.graphToCanvasCoords(
            coords[index + 2],
            coords[index + 3]
        );
