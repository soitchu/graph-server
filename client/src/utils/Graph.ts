...
type GraphEvent = "resize" | "mouseMove" | "mouseDown" | "mouseUp";

export class Graph {
  events: { [key in GraphEvent]: Function } = {
    resize: undefined,
    mouseMove: undefined,
    mouseDown: undefined,
    mouseUp: undefined,
  ...
  toDrawGraphs: { [key in GraphModes["type"]]: GraphModes[] } = {
    drawGraphX: [] as GraphXY[],
    drawLines: [] as GraphLineInter[],
    drawPoints: [] as GraphPoint[],
  };
  ...
  backCanvasCtx: OffscreenCanvasRenderingContext2D;

  numberCanvas: HTMLCanvasElement;
  numberCtx: CanvasRenderingContext2D;

  tmpCanvas: OffscreenCanvas;
  tmpCanvasCtx: OffscreenCanvasRenderingContext2D;
  ...
  constructor(
    ...
    width: number,
    drawY = false,
    coordinatesScale?: { x: number; y: number }
  ) {
    if (coordinatesScale) {
  ...
  on(event: GraphEvent, func: Function) {
  ...
  drawLines() {
    const graphConfig = this.toDrawGraphs.drawLines as GraphLine[];
  ...
  drawPoints() {
    ...
    const strengths = [11, 22, 11, 22, 22, 22, 11, 22, 11];
    ...
    const points = this.toDrawGraphs.drawPoints as GraphPoint[];
  ...
  drawGraphX() {
    const graphConfig = this.toDrawGraphs.drawGraphX as GraphXY[];
  ...
  canvasToGraphCoordsScaled(x: number, y: number) {
    return [
      x / this.scale - this.translate.x,
  ...
  graphToCanvasCoordsScaled(x: number, y: number) {
    return [
      (x + this.translate.x) * this.scale,
  ...
  scaleUp(inc: number, x = 0, y = 0, shouldRedraw = true) {
  ...
  getMass() {
    console.log(this);
    const imageData = this.frontCtx.getImageData(
      0,
      0,
      this.frontCanvas.width,
      this.frontCanvas.height
  ...
  drawNums() {
    ...
    const decimalPlaces = Math.ceil(
      Math.abs(Math.log10(Math.abs(xAxisNums[1][0] - xAxisNums[0][0])))
    );
    ...
    for (var i = 0; i < yAxisNums.length; i++) {
      let val: number | string = yAxisNums[i][0] / this.scaleY;
  ...
  drawGrid() {
    ...
    const distanceUnrounded =
      this.width / Math.floor(partitionsIdeally) / this.scale;
    const distanceRounded = Math.pow(
      10,
      Math.floor(Math.log10(distanceUnrounded))
    );

    const distanceInCanvasCoords =
      Math.floor(distanceUnrounded / distanceRounded) * distanceRounded;
    ...
    const canvasOrigin = [this.translate.x, this.translate.y];

    // Canvas coords where the grid starts
    const graphRoundedStart = [
      Math.floor(graphOrigin[0] / distanceInGraphCoords) *
        distanceInGraphCoords,
      Math.floor(graphOrigin[1] / distanceInGraphCoords) *
        distanceInGraphCoords,
    ];

    const canvasRoundedStart = [
      Math.floor(canvasOrigin[0] / distanceInCanvasCoords) *
        distanceInCanvasCoords,
      Math.floor(canvasOrigin[1] / distanceInCanvasCoords) *
        distanceInCanvasCoords,
    ];
      ...
      for (var i = 0; i <= partitions[k]; i++) {
        for (var j = 1; j < 4; j++) {
          ...
          if (isX) {
            ...
            ctx.lineTo(
              canvasCoords[k] + (distanceInGraphCoords / 4) * j,
              this.frontCanvas.height
            );
          } else {
        ...
        numArray.push([graphCoords[k], canvasCoords[k]]);
  ...
  updateBackCanvas() {
    ...
    this.state.graph.bottomRight = this.canvasToGraphCoords(
      this.frontCanvas.width,
      this.frontCanvas.height
    );

    this.backCanvasCtx.clearRect(
      0,
      0,
      this.frontCanvas.width,
      this.frontCanvas.height
  ...
  move(event: MouseEvent) {
    ...
    if (this.state.mouseDown) {
      ...
      this.translate.x =
        this.translate.iniX - (this.state.mouseDownCoords.x - x) / this.scale;
      this.translate.y =
        this.translate.iniY - (this.state.mouseDownCoords.y - y) / this.scale;
